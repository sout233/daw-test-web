...
function ResultPage() {
  ...
  const listItems = keys.map((key) => {
    const percent = (
      ((sortedDictionary[key] - minValue) / (maxValue - minValue)) *
      100
    ).toFixed(2);
    const rating = parseInt(sortedDictionary[key] - minValue);

    return (
      <div className="w-72" key={aimDaw}>
  ...
  let [isDamn, setIsDamn] = useState(false);
  ...
  return (
    <div
      ref={captureRef}
      className="max-w-4xl w-auto overscroll-x-none relative overflow-hidden flex flex-col items-center justify-center m-auto bg-base-100"
    >
      <div className="flex flex-col justify-center">
        <div className="absolute translate-y-[30%]">
          ...
          <img
            src={logos[aimDaw]}
            alt="logo"
            className="w-60 rotate-6 rounded-2xl translate-x-[120%] translate-y-[10%] opacity-50"
          />
        </div>
        <div className="m-10">
          <div className="flex flex-row justify-between">
            <h1 className="md:text-2xl text-xl font-bold text-base-content">
              测测哪款DAW更适合你
            </h1>
            <h1 className="md:text-2xl text-xl font-bold text-primary italic">
              测试结果
            </h1>
          </div>
          ...
          <div className="flex flex-row justify-between">
            {isDamn && (
              <div className="flex flex-col">
                <h1 className="md:text-4xl text-3xl font-bold text-base-content">
                  或许
                </h1>
                <h1 className="md:text-4xl text-3xl font-bold text-base-content">
                  <span className="text-primary">{aimDaw}</span> 比较适合你？
                </h1>
                <h1 className="text-xl font-bold text-base-content">
                  其次是{sortedEntries[1][0]}和{sortedEntries[2][0]}
                </h1>
                <h1 className="text-xl font-bold text-base-content">
                  它们可能也是不错的选择
                </h1>
      ...
      {isDamn && (
        <div className="flex flex-col justify-between items-center">
          ...
          <h1 className="text-6xl font-bold text-primary italic truncate absolute left-[50%] translate-x-[-50%] translate-y-[-50%] opacity-20 text-nowrap">
            {recommend && recommend[aimDaw]["tag"]}
          </h1>
          <p className="pt-5 text-lg p-10 font-bold text-base-content text-pretty break-before-auto whitespace-pre-line text-center">
            {recommend && recommend[aimDaw]["desc"]}
          </p>
        </div>
      )}
      <div className="h-20"></div>
      ...
      <div className="items-center justify-center text-center">
        ...
        {recommend && isDamn && <Radar {...config}></Radar>}
      </div>
      ...
      <div
        ref={buttonRef}
        className="class2hide flex flex-col justify-center items-center"
      >
        <button className="btn btn-primary w-60" onClick={handleCaptureClick}>
          生成报告
        </button>
        <p className="text-sm text-base-content opacity-50 p-2">
          可能会有问题，还是截屏吧（
        </p>
      </div>
