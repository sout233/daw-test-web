...
function MainExam() {
    ...
    function submitButtonClick() {
        ...
        for (let i = 0; i < qs.length; i++) {
            let qValue = qs[i].getElementsByClassName("range")[0].attributes["data"].value;
    ...
    return (
        <>
            <div className="flex flex-col justify-center items-center h-28 bg-primary">
                ...
                <h1 className="text-primary-content mt-2">共20道题，请根据实际情况作答</h1>
            </div>
                ...
                <div className="m-4 space-y-14 max-w-xl items-center p-4 bg-base-200 rounded-lg shadow-lg">
                    <div ref={questionListRef}>
                        {questions && questions.map(
                            (question, index) =>
                                (<Question key={question.question} title={question.question} index={index} changeRating={changeRating}></Question>)
                        )}
                    </div>
                    <button className="btn w-full px-10 btn-primary" onClick={submitButtonClick}>OK! 我好了</button>
                </div>
                <div className="h-4"></div>
            </div>
        </>)
